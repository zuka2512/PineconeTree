...
const getNodesByParentId = (parentId: number, callback: Function) => {
  db.all(
    `SELECT * FROM nodes WHERE parentId = ? ORDER BY ordering`,
    [parentId],
    (err, rows) => {
      if (err) {
        callback(err);
...
// Endpoint za dohvat stabla ili svih čvorova
app.get("/tree/:parentId?", (req, res) => {
  const parentId = req.params.parentId ? parseInt(req.params.parentId) : null;

  // Ako parentId nije dostavljen, dohvatit ćemo sve čvorove
  if (parentId === null) {
    db.all(`SELECT * FROM nodes ORDER BY ordering`, (err, nodes) => {
      if (err) {
...
app.post("/node", (req, res) => {
  ...
  db.get(
    `SELECT MAX(ordering) as maxOrder FROM nodes WHERE parentId = ?`,
    [parentId],
    (err, row: { maxOrder: number | null }) => {
      const newOrder = (row?.maxOrder || 0) + 1;
      db.run(
        `INSERT INTO nodes (parentId, title, ordering) VALUES (?, ?, ?)`,
        [parentId, title, newOrder],
        function (err) {
          if (err) {
            res.status(500).json({ error: err.message });
          } else {
...
app.delete("/node/:id", (req: any, res: any) => {
  ...
  DELETE FROM nodes WHERE id IN (SELECT id FROM sub_tree)`,
    [id],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
...
app.patch("/node/:id/move", (req: any, res: any) => {
  ...
  /* Možda poslije izbrisati
  if (parseInt(id) === 0 || parseInt(newParentId) === parseInt(id)) {
    return res.status(400).json({ error: "Invalid move" });
  }*/
  ...
  db.get(
    ...
    (err, row: { maxOrder: number | null }) => {
      const newOrder = (row?.maxOrder || 0) + 1;
      db.run(
        `UPDATE nodes SET parentId = ?, ordering = ? WHERE id = ?`,
...
app.patch("/node/:id/reorder", (req, res) => {
  ...
  db.run(
    `UPDATE nodes SET ordering = ? WHERE id = ?`,
    [newOrder, id],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
